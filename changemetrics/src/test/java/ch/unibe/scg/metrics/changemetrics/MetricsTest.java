...
public class MetricsTest {
	...
	private static ChangeMetrics changeMetrics;
	private static Map<String, CommitRange> ranges;
	
	private static CMRepository repository;
	...
	public static void readPath() throws FileNotFoundException {
		...
		changeMetrics = new ChangeMetrics(path);
		changeMetrics.setEveryNthCommit(1);
		changeMetrics.setRange(COMMIT, COMMIT);
		ranges = changeMetrics.generateCommitListWithWeeks(999999); // till the beginning of the repo
		
		CommitRange range = ranges.get(COMMIT);
		
		changeMetrics.setRange(range);
		repository = changeMetrics.analyze();
	}
